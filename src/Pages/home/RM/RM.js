...
export const RM = ({ children, RmPlay }) => {
	const svg = React.useRef();
	const eyeLeftClosed = React.useRef();
	const eye_left_lids = React.useRef();
	const eyeRightInner = React.useRef();
	const eye_right_propil = React.useRef();
	const eye_right_Inner_path = React.useRef();
	const eye_right_lids = React.useRef();
	const ear_browe = React.useRef();
	const hair = React.useRef();

	const [timeline, setTimeline] = useState();
	useEffect(() => {
		setTimeline(gsap.timeline({ repeat: -1, repeatDelay: 3 }));
	}, []);

	useEffect(() => {
		if (RmPlay !== undefined && timeline !== undefined) {
			RmPlay ? timeline.play() : timeline.pause();
		}
	}, [RmPlay, timeline]);
	...
	React.useEffect(() => {
		if (timeline) {
			...
			if (tl) {
				tl.set(eyeLeftClosed.current, { opacity: 1 });
				tl.to(eyeLeftClosed.current, { opacity: 0, yoyo: true, duration: 0.6 });
			}
		}
	}, [timeline]);
	//
	let TlBlinkOnHover = useRef(null);
	useEffect(() => {
		// let tlRickMorthy = gsap.timeline({ repeat: -1, repeatDelay: 5 });
		let tlRickMorthy = gsap.timeline({});
		TlBlinkOnHover.current = tlRickMorthy;
	}, []);
	// React.useEffect(() => {
	// 	let tl = TlBlinkOnHover.current;
	// 	// tl_eye_right_propil.current && tl_eye_right_propil.current.progress(0).kill();
	// 	// tl_eye_right_propil.current = gsap.timeline({ repeat: -1 });
	// 	// let tl_eye_right_propil = tl_eye_right_propil.current
	// 	const duration = 0.2;
	// 	if (isShown) {
	// 		tl.set(eyeLeftClosed.current, { opacity: 0 });
	// 		tl.to(eye_left_lids.current, { opacity: 0, duration: duration }, 0);
	// 		tl.to(eye_right_lids.current, { opacity: 0, duration: duration }, 0);
	// 		tl.to(eye_right_Inner_path.current, { fill: "rgb(148, 185, 255)", duration: duration }, 0);
	// 		tl.to(ear_browe.current, { rotation: 360, y: "-20px", duration: duration, transformOrigin: "50% 50%" }, 0);
	// 		tl.to(hair.current, { scale: "1.1", transformOrigin: "center center", duration: duration }, 0);
	// 		// console.log(tl);
	// 		//tl_eye_right_propil.current.to(eye_right_propil.current, { scale: "1.5", duration: 0.5 });
	// 		// timeline.seek(0);
	// 		// timeline.pause();
	// 	} else {
	// 		tl.to(eye_left_lids.current, { opacity: 1, duration: duration }, 0);
	// 		tl.to(eye_right_lids.current, { opacity: 1, duration: duration }, 0);
	// 		tl.to(eye_right_Inner_path.current, { fill: "white", duration: duration }, 0);
	// 		tl.to(ear_browe.current, { rotation: 0, y: "0px", duration: duration, transformOrigin: "50% 50%" }, 0);
	// 		tl.to(hair.current, { scale: "1", transformOrigin: "center center", duration: duration }, 0);
	// 		//timeline.resume();
	// 		// right eye propil
	// 		// tl_eye_right_propil.current.repeat(0);
	// 		// tl_eye_right_propil.current.set(eye_right_propil.current, { scale: 1 }, 0);
	// 	}
	// }, [isShown, TlBlinkOnHover]);
...
L 298.1 176.5
Q 297.70625 176.51875 297.3 176.55 297.1 176.55 296.9 176.55 296.6 176.55 296.35 176.55 296.15 176.55 295.95 176.55
L 292.95 176.7
Q 288.4375 177.425 283.75 180.2 270.3 188.25 268.5 210.15
L 268.85 213
Q 269.45 216.6 271 220 275.85 231 287.8 236.65
L 290.75 237.15
Q 291.6875 237.2625 292.6 237.35
L 295.75 237.45
...
M 298.125 176.5
Q 297.5033203125 176.514453125 296.9 176.55 296.4 176.5 295.95 176.55
M 292.95 176.725
Q 288.2359375 177.321484375 283.35 180.2 269.9 188.25 268.1 210.15
L 268.45 213
Q 269.05 216.6 270.6 220 275.45 231 287.4 236.65
L 290.35 237.15
Q 291.4560546875 237.2685546875 292.625 237.35"
						/>
					</g>
				</g>
			</g>

			<g id="eye_right_propil" ref={eye_right_propil}>
				<g>
					<g>
						<g>
							<path
...
L 307.45 209.2 307.4 209.35 Z"
			...
			<g id="eye_right_lids" ref={eye_right_lids}>
				<g>
					<g>
						<g>
							<path
...
Q 246.75 214.55 246.85 213.25 246.85 212.1 246.7 211.8 246.45 211.25 245.75 210.85 244.6 210 243.6 209.8 242.1 209.55 241.4 210.55 241.2 210.8 240.9 211.65 240.7 212.3 240.55 212.9 240.45 213.95 240.9 214.5 241.3 214.9 242.3 215.15 243.8 215.65 244.35 215.75 245.75 215.85 246.35 215.1 Z"
			...
			<g id="eye_left_lids" ref={eye_left_lids}>
				<g>
					<g>
						<g>
							<path
...
Q 250.65 208.45 264 211.15 263.7 208.75 262.75 205.2 260.9 198.05 257.8 192.3 256.45 190.1 253.9 187.25 248.8 181.55 242.8 178.2 238.95 176.95 231.8 176.5 227.95 177 223.1 178.65 213.4 182 208.45 187.8 205.5 191.35 202.65 196.2 196.9 205.9 197.3 212.25 Z"
			...
			<g id="eye_left_closed" ref={eyeLeftClosed} style={{ opacity: 0 }}>
				<g>
					<g>
						<g>
							<path
